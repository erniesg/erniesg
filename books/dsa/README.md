...
`books/dsa/workspace/`, tier-by-tier grader output, XP, badges, and streaks.
...
Use `python3 books/dsa/tools/reader.py --no-open` when you do not want it to open a
browser automatically. Press Ctrl-C in the terminal to stop the reader.

### Export the EPUB

```bash
python3 books/dsa/tools/export.py
```

The standards-based EPUB 3 file is written to `books/dsa/dist/`. It contains the
...
```bash
python3 books/dsa/tools/runner.py list          # what's available, what's green
python3 books/dsa/tools/runner.py start ch01    # copy starter into your workspace
# read books/dsa/chapters/ch01-*/chapter.md, then edit books/dsa/workspace/ch01/*.py
python3 books/dsa/tools/runner.py run ch01      # grade tier by tier
python3 books/dsa/tools/runner.py status        # XP, badges, streak
```

Your code lives in `books/dsa/workspace/` (gitignored). The grader never edits
...
```bash
python3 books/dsa/tools/test_runner.py          # the grader's own unit tests
python3 books/dsa/tools/runner.py verify ch01   # reference solution passes all tiers
```
