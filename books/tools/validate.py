...
from render import BOOKS, CHAPTERS, EXERCISE_PARTS, FIGURES as FIGURES_DIR
from render import book_files, node_files, parse_exercise, same_output, split_blocks
...
def fail(where: Path, message: str) -> None:
    problems.append(f"{where.relative_to(BOOKS.parent)}: {message}")
...
def check_node(path: Path, meta: dict, body: str) -> None:
    ...
    if figure and not (FIGURES_DIR / f"{figure}.json").is_file():
        fail(path, f"figure `{figure}` has no file in books/figures/")
