...
Same list, same staff. This whole difference is why DSA matters.
...
Four tiers grade every challenge, stopping at the first failure. **Public** is
the statement's examples. **Edge** is empty, single, all-identical, the largest
legal value. **Stress** runs your code against an obviously-correct slow one on
random inputs, and is the tier that finds what you didn't imagine. **Perf** is
an input big enough that a correct-but-slow answer fails.
