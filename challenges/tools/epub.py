...
"""Build the EPUB from the same nodes the web edition reads.
    ...
    python3 challenges/tools/epub.py            -> challenges/dist/<slug>-<edition>.epub
...
from render import PRINT_CSS, load_book, render_node

CHALLENGES = Path(__file__).resolve().parent.parent
DIST = CHALLENGES / "dist"
...
def build() -> Path:
    ...
    slug = tomllib.loads((CHALLENGES / "book.toml").read_text()).get("slug", "book")
