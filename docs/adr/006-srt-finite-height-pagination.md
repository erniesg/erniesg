...
Layout manifest version `1.2.0` embeds the pagination policy summary and node policy. A finite placement must name a valid page; flow placements also name region and page/column span. Fragment records carry explicit previous/next lineage. Current-region stability is recorded as the stable deterministic fragment prefix completed before later regions; final page count is a separate metric.
