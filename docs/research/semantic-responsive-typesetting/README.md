...
Authoritative e-ink device geometry comes from the target registry: the 7.3-inch Paper Pro Move is `954 × 1696` device pixels at `264 PPI`, and the 11.8-inch Paper Pro is `1620 × 2160` device pixels at `229 PPI`. Browser previews may scale those viewports, but must retain their aspect ratio, profile/version receipt, semantic reading order, and honest distinction between publisher-controlled output and reader-controlled EPUB behavior.
