...
and rejects a v1.6 receipt unless compatibility is deliberately enabled. When
an OCR run is compared with either another v1.6 run or a frozen v1.5 baseline,
add `--corpus-report-schema-policy v1.5-v1.6-compatible`. The resulting
comparison uses schema `1.6.0` and records the explicit policy and both exact
audit schema version/path pairs. The v1.6 OCR field is validated but is not
itself a directional quality metric; OCR effects remain visible through
completeness, readiness, diagnostics, structure, and artifacts.
