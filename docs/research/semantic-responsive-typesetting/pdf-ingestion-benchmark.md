...
Without that external accepted baseline and its independently recorded canonical receipt digest, `baselineComparison.status` is `not-configured`, top-level `passed` is false, and the runner exits nonzero. As of 2026-07-21, the latest owner-local named repeat is `localValidationPassed`: it replays the pinned decision set, reconstructs twice, and produces valid Mobile, Paper Pro Move, and Paper Pro artifacts on both repetitions. That is local candidate evidence, not an accepted baseline or clean-VM exact-head evidence, so issue 032 remains open.
...
- The latest owner-local named repeat replays decision set SHA-256 `eeb2a29234f1416fe0a1d96d16cc770978ba68c169869db98323453114576f26`. Both fresh reconstructions and all six selected-profile artifacts are deterministic; all six pass structural validation and EPUBCheck. The local receipt reports complete source-text coverage, all 51 source assets and relationships packaged, no unresolved semantic objects, no corrupting joins, and `localValidationPassed: true`. There is still no external human-accepted baseline, and this evidence was not produced from a clean trusted-VM exact head. A direct-URL browser import deliberately remains review-required until its separately supplied local decision JSON is imported; it must not silently inherit owner-side adjudication.
