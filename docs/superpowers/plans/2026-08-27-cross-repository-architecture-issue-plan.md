...
The map retains the completed GitHub creation order as history. It is not execution authority. `depends_on`, `phase_zero_runtime_gate`, each decision gate's structured `admission_condition`, and `recommended_full_dag_order` are the corrected execution contract. The full order contains all 25 issue nodes plus E-ACT, A-REQ, and A-ACT; in particular, S-02 cannot start until A-01 lands, and E-06 must fail admission unless a current accepted E-ACT receipt satisfies the machine-readable binding contract. Before accepting any E-ACT, A-REQ, or A-ACT receipt, the coordinator must authenticate and validate new immutable explicit-user-authorization evidence bound to that exact named action and target, authorized execution actor, expiry, and explicit cost/billing scope. The user's prior `go` authorized only the reviewed ADR/context and issue creation/linking; it cannot satisfy any future decision gate.

## Already-created issue reconciliation at promotion

Struct issue [#5](https://github.com/erniesg/struct/issues/5) was created before the phase-0 ordering repair. Its current body lists only S-01 and links the immutable `d277971` plan/ADR. The sole coordinator must replace that body after the repaired commit receives fresh exact-head review and is promoted. Until the body is reconciled, #5 is blocked and cannot be admitted for implementation. No worker may treat its older body as authority.

The exact replacement body is the following template, with `PROMOTED_REPAIR_SHA` replaced in both source links by the exact reviewed commit the coordinator promotes:

```markdown
## Architecture work item: S-02

Canonical scope, exclusions, acceptance criteria, and review routing are defined in the [approved architecture issue plan](https://github.com/erniesg/erniesg/blob/PROMOTED_REPAIR_SHA/docs/superpowers/plans/2026-08-27-cross-repository-architecture-issue-plan.md); search that document for `S-02`. The governing [ADR-0001](https://github.com/erniesg/erniesg/blob/PROMOTED_REPAIR_SHA/docs/superpowers/specs/2026-08-27-cross-repository-domain-ownership-and-package-boundaries.md) is the source of domain-boundary and safety decisions.

## Dependencies

- [S-01](https://github.com/erniesg/struct/issues/4)
- [A-01](https://github.com/erniesg/aether/issues/182) — transitively proves the complete S-01 -> E-01 -> A-01 phase-0 documentation spine

## Execution guard

Before implementation, record the current immutable base, confirm both dependencies landed, reserve owned seams, validate intervening default changes, and obtain the required independent exact-head reviews. This issue does not authorize publishing, deployment, route activation, visibility changes, direct printer contact, or bypassing a named decision gate.
```
...
- External assets require the versioned isolated streaming resolver executor. Side-effect-free allocation atomically reserves a collision-resistant supervisor-generated execution ID that is unique across active and retired executions and never reused during the supervisor lifetime, then returns one-shot supervisor control and idempotent cleanup before the sole authority-granting `start()` call. Concurrent duplicate allocation or retired-ID reuse fails before spawn/grant. Startup is budgeted, revocable, and cleanable even when it throws, rejects, or never settles. `start()` succeeds at most once and is permanently unavailable after revocation, cleanup, or termination begins. Accepted revocation requires atomically closed future grants plus `{capabilitiesRevoked: true, quarantined: true}` bound to the exact active ID reservation; the terminal receipt binds that same reservation, and valid termination atomically converts it to an immutable retired-ID tombstone before capacity reuse. Required logical href/content address, request/result/profile versions, transport-kind-discriminated non-secret receipts, trusted endpoint/root/store equality, synchronous authority revocation, pre-start supervisor cleanup, optional iterator return only after a result exists, out-of-realm termination/join on every outcome, package-monotonic total/cleanup budgets, abort -> revoke -> conditional iterator return -> control cleanup -> terminate ordering, length caps, and residual work are normative and covered by the conformance harness.
...
- Wrong media/bundle/schema versions, digest, receipt, base64, asset set, metadata, bytes, resource ID, transport-kind policy/receipt equality, execution/revocation/cleanup/terminal receipt, and resource limits fail closed. Concurrent duplicate execution IDs, retired-ID reuse, duplicate start, start after revocation/cleanup/termination, synchronous startup throw, rejected startup, never-settling startup, late startup completion after timeout/revocation, quarantined-false revocation, revocation failure, mismatched execution IDs, stale revocation/terminal receipts swapped across active or retired executions, and clean successful completion prove the lifetime-unique one-shot state machine, pre-start cleanup, terminal join, zero post-revocation broker calls/grants/descendants, and permanent ID/capacity no-reuse behavior. Duplicate or retired IDs fail before authority grant; identity ambiguity discovered later forces parent-proven fail-stop with implicated capacity retired. Never-settling cleanup/join is exercised in a sacrificial verification host whose parent proves bounded fail-stop, zero capacity reuse, and aggregate-pressure admission limits.
...
**Depends on:** E-05 exact-head implementation and independent review; separately authorized package artifact/version; a new target-specific immutable explicit-user-authorization record. Prior ADR/issue-creation approval is ineligible.

**Decision packet:** Immutable authorization ID and evidence SHA-256, authenticated authorizing-user identity, exact action `activate Ernie.SG writer/public route`, exact target environment/route/store, authorized execution actor, issue time, expiry, explicit no-cost/no-billing declaration or currency/hard cap/billing owner/permitted charge category, exact repository head, Struct package/artifact/schema pins, migration and backfill output, validation evidence, concurrency/recovery results, rollout cohort, observation window and thresholds, rollback or forward-recovery constraints, and abort conditions.

**Acceptance:** The user explicitly authorizes the named writer/public-route activation at the named immutable inputs. Before accepting a receipt, the coordinator authenticates the authorizing user, verifies that immutable authorization evidence matches the exact action, target, actor, expiry, and cost/billing scope, and rejects reuse of the earlier ADR/issue `go`. Only then may the coordinator record a current `erniesg.writer-route-activation.v1` receipt with `status: accepted`, the authorization-evidence SHA-256, and every binding named by the map's E-ACT `admission_condition`. E-06 admission validates that receipt before any implementation or mutation; missing, mutable, unauthenticated, stale, replayed, mismatched, expired, failed-threshold, or unsafe-downgrade evidence denies admission. E-06 cannot create or self-satisfy this gate.
...
**Depends on:** A-01 exact-head product contract and a new immutable explicit-user-authorization record for the exact inquiry. Prior ADR/issue-creation approval is ineligible. The gate is unnecessary when complete requirements are user-supplied or gathered from public material without contact.

**Decision packet:** Immutable authorization ID and evidence SHA-256, authenticated authorizing-user identity, exact action `send printer-requirements inquiry`, exact named printer/contact/channel target, authorized execution actor, exact questions, disclosed project/files/metadata, communication authority, issue time, expiry, explicit no-cost/no-billing declaration or currency/hard cap/billing owner/permitted charge category, and evidence-retention location. It authorizes requirements discovery only—not artifact submission, proof approval, ordering, purchase, or deployment.

**Acceptance:** Before accepting `aether.printer-requirements-inquiry.v1`, the coordinator authenticates the authorizing user and validates the immutable authorization-evidence digest, exact action, target/contact/channel, actor, expiry, disclosures, and cost/billing scope; prior `go`, missing evidence, or any mismatch denies the inquiry. The accepted receipt binds that authorization digest. Only the named inquiry may be sent. The resulting versioned PDF/X, ICC, font, bleed, resolution, imposition, binding, and acceptance requirements are retained with source/date evidence. Changed contact, disclosure, cost, actor, or purpose invalidates the gate. A-REQ cannot authorize A-02 implementation or A-ACT actions.
...
- The Aether executor maps only the digest-derived non-secret resource ID through trusted endpoint/root configuration. Side-effect-free allocation atomically reserves a collision-resistant supervisor-generated execution ID unique across active and retired executions and never reused for the supervisor lifetime, then returns one-shot independently revocable supervisor control and idempotent cleanup before authority-granting startup; cleanup remains callable after synchronous throw, rejection, or never-settling startup, and clean success also terminates/joins with a receipt bound to the exact active reservation and atomically converts it to an immutable retired-ID tombstone before capacity reuse. It passes the Struct conformance harness plus concurrent duplicate IDs, retired-ID reuse, duplicate/late start, start-after-revocation/cleanup/termination, late completion after timeout/revocation, quarantined-false, revocation-failure, mismatched execution-ID, stale revocation/terminal receipts swapped across active or retired executions, zero post-revocation broker-call/grant/descendant, domain-separated policy-digest, endpoint/path alias, secret URL/path/matrix/percent-encoding non-retention, redirect, DNS/private-target, traversal/symlink, monotonic-budget/cancellation, deceptive-length, concurrency, synchronous authority revocation, pre-start cleanup, conditional iterator return, out-of-realm hard termination/join, transport-kind receipt equality, terminal-receipt, sacrificial-host fail-stop with implicated capacity retired, repeated-failure admission, and residual-work tests. Duplicate/reused IDs fail before authority grant or force parent-proven fail-stop if detected later.
...
**Depends on:** A-02 exact-head inactive proof and independent review; a new immutable explicit-user-authorization record for each named external action. Prior ADR/issue-creation approval is ineligible.

**Decision packet:** Immutable authorization ID and evidence SHA-256, authenticated authorizing-user identity, one exact action (`activate Aether route`, `deploy Aether`, or `submit Aether printer proof`), exact route/environment or printer/contact target, authorized execution actor, issue time, expiry, explicit no-cost/no-billing declaration or currency/hard cap/billing owner/permitted charge category, exact repository head and artifacts, audience, actor/session/workspace authorization policy and negative-test evidence, import pin, trust root, rollback-detecting/online status mode, minimum acceptable trust-policy epoch/witness and freshness, renderer/profile, health/rollback thresholds, and—only for printer submission—the disclosed files/metadata, purpose, acceptance criteria, and communication authority.

**Acceptance:** Before accepting `aether.external-action.v1`, the coordinator authenticates the authorizing user and validates the immutable authorization-evidence digest, exact single action, exact target, actor, expiry, and cost/billing scope; prior `go`, missing evidence, or any mismatch denies the action. The accepted receipt binds that authorization digest. Only the action and target explicitly authorized in the current packet may occur; a route activation cannot authorize deployment or printer submission. Activation proves unauthorized reads, writes, imports, and cross-workspace access still fail at the exact head. An expired/stale head, changed artifact/profile, trust-policy rollback/staleness, ambiguous audience, unapproved cost/outreach, or failed health threshold invalidates the gate. A-02 cannot create or self-satisfy it; user-supplied printer evidence may be recorded without external outreach.
...
| Writer/public route activation authorized | Newly authenticated immutable explicit-user authorization + digest-bound E-ACT receipt for exact action/target/actor/expiry/cost scope, head, pins, thresholds, and recovery; prior ADR/issue `go` is ineligible | E-06 |
...
| Aether requirements outreach authorized | Newly authenticated immutable explicit-user authorization + digest-bound A-REQ receipt for exact inquiry/target/actor/expiry/disclosure/cost scope; prior ADR/issue `go` is ineligible | Only the named inquiry |
...
| Aether external action authorized | Newly authenticated immutable explicit-user authorization + digest-bound A-ACT receipt for one exact action/target/actor/expiry/cost scope, head/artifacts, thresholds, and recovery; prior ADR/issue `go` is ineligible | Only the named route/deployment/submission action |
