...
| Artifact | Initial reviewed SHA-256 at `d277971` | First PR repair SHA-256 at `d657be7` | Second PR repair SHA-256 at `cd3e9b2` | Round-3 repair SHA-256 at `7b3a0bd` | Current focused repair SHA-256 |
|---|---|---|---|---|---|
| `docs/superpowers/specs/2026-08-27-cross-repository-domain-ownership-and-package-boundaries.md` | `7778a727fdd9fa4652bfdb71cd571e3402b6c42ff7a8e07824b477b6a5497f78` | `a6fa64dde853d58f3f2291a2c232b9d45cb1e1d53b4ef205f85551b46a21282d` | `6e5e0e29bb8ef916714b638af5e8134dcb31424beb9386a07c73d1f6ffd71a74` | `19df427eec1f4e6228f280c60fc080d3a2dca0dbedac326a13592a5f974c0106` | `19df427eec1f4e6228f280c60fc080d3a2dca0dbedac326a13592a5f974c0106` |
| `docs/superpowers/plans/2026-08-27-cross-repository-architecture-issue-plan.md` | `667db6ac6063751fef3cc6f6c2addc6eee132025ba433278bd43075e1f64410f` | `ece2d576ac20c382791cd88ab551faf4466e87a78e95d0e759bd94011adb335a` | `4a2502b2e288d23e22d421ead5f85d0b5050d7a7612612bb23bbfae1da9760d3` | `7fb71f8dd9b06157467fa16710b535d3d826ada844f31c85dd728a30f19321e7` | `6a7f9fdf66a06ae43df50a38625fbd2419b11d6dcf529ad8d963246f023363ff` |
| `docs/superpowers/plans/2026-08-27-cross-repository-architecture-issue-map.json` | `3c4245ac24443b9e138959afecbcb8c41d0dc3b2dab775d997605d82421f7e53` | `19a8146d02b2c0c17906d17f32b62c4ff5fc614a3943af9e87c6c6ccc3f406c2` | `771f9295bf9a506ce7d2b375610179a279d0abd40807367a93744bcba6252411` | `980d09ff1e48bb0c0e3f69319b512f0d7c79ea4cc0596f552067df6bcab550a3` | `080c8591377c567207fc021dbc2e96afcfb60294767484089616384de44129d6` |

The initial ADR and issue plan were byte-for-byte copies of the final pre-PR reviewed local artifacts. Independent fresh-context architecture, security, dependency-order, and exact-head evidence passes repaired every actionable P0–P2 finding through `d277971`. Commit `d657be7` recorded the first PR review repair. Commit `cd3e9b2` added the supervisor-enforced one-shot resolver state machine, the full issue-plus-decision-gate order, the mandatory coordinator reconciliation for already-created Struct #5, and corrected GFM evidence tables. Commit `7b3a0bd` additionally required immutable explicit-user-authorization evidence before any decision-gate receipt is accepted and made execution IDs collision-resistant, unique across active and retired executions, and non-reusable for the supervisor lifetime. The current hashes align the plan's A-ACT action vocabulary exactly with the map's canonical literals; they require another fresh exact-head review before push/merge.
...
The user's explicit `go` authorized the sole coordinator to persist the reviewed context, push the reviewed documentation commits, and create/link the mapped GitHub issues after rechecking exact remote bases. The coordinator completed that planning mutation on 2026-08-27: Struct #4–#7, Ernie.SG #280–#287, Aether #182–#183, and Rucksack #674–#684. It did not authorize this task to mutate remotes and did not authorize implementation, package publication, deployment, merge, printer contact, E-ACT, A-REQ, or A-ACT. Each future gate requires its own newly authenticated immutable explicit-user-authorization evidence bound to the exact action, target, execution actor, expiry, and cost/billing scope and validated before receipt acceptance; this planning `go` is explicitly ineligible.
...
- PR #279 repair map checks: JSON parses; all 25 issues have unique durable GitHub identities; the full 28-node order contains all 25 issues plus E-ACT, A-REQ, and A-ACT and respects every dependency. Each gate structurally requires immutable authenticated explicit-user authorization, exact action/target/actor/expiry/cost bindings, authorization-digest binding in its accepted receipt, validation before receipt acceptance, and exclusion of the prior ADR/issue `go`; E-ACT remains before E-06. The original GitHub creation order is retained only as completed history. A live read-only GitHub check found zero title, URL, or recorded-open-state mismatches across all 25 issues. Struct #5 remains execution-blocked until the coordinator replaces its d277-only body using the exact promoted-repair template in the plan.
- Cross-document A-ACT action-enum assertion: the three backtick-delimited action literals in the plan decision packet exactly equal the map's `allowed_exact_named_actions` array in order and spelling.
- Resolver lifecycle structural checks: the ADR, S-03, and A-02 require collision-resistant supervisor-generated IDs, an atomic active-plus-retired lifetime registry, no retired-ID reuse, concurrent-duplicate and swapped-stale-receipt cases, pre-grant rejection, and parent-proven fail-stop with implicated capacity retired when ambiguity is discovered later.
- GFM rendering validation: the installed GFM parser and Markdown-to-HTML pipeline rendered all 14 tables across the ADR, plan, and evidence record with uniform rows and widths `2,2,2,3,3,3,3,3,3,4,4,4,5,6`.
- `git diff --check`: passed.
- Promoted-document hash verification: passed at the exact hashes recorded above.
- Round-3 and focused-repair explicit and commit-time secret scans: passed.
...
The repository evidence command therefore returned its honest `failed` classification rather than masking the host limitation. The documentation-only diff introduces no executable path and reproduces the exact-base test result. Independent exact-commit review covered `d277971422476ff7683392c1254267ceb63c6fe4`; the first PR review repair landed locally at `d657be7dab522df35e773754bb117ee670bdcf38`. Fresh exact-head review of `d657be7` identified reusable-start/revocation, decision-gate ordering, already-created #5 drift, and two GFM delimiter findings; `cd3e9b2bc4f69f6f92b3d00901eca39bf9c0586d` repaired them. Round-3 review of `cd3e9b2` identified missing immutable explicit-user-authorization evidence in decision-gate admission and missing supervisor-lifetime execution-ID uniqueness; `7b3a0bde7d7fec9e78a9cc0ffb105b1c864e4442` repaired both. The focused follow-up aligns the remaining A-ACT plan literals with the canonical map enum while retaining every prior lifecycle, ordering, reconciliation, authorization-history, and GFM correction. The coordinator must obtain another fresh exact-head review and revalidate remote defaults/open work before pushing or merging this follow-up or performing the recorded #5 reconciliation.
