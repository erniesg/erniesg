...
interface StructResolverExecutionControl {
  // Allocated once by the supervisor and shared by every receipt for this execution.
...
The package measures the total budget with a monotonic clock and reserves `cleanupReserveMs` before allocation. `allocate()` may construct supervisor state but cannot start adapter code or grant credential/filesystem/network authority. It allocates one supervisor execution ID and returns supervisor-owned `control`, including idempotent `cleanup()`, before Struct invokes `start()`; every accepted revocation and terminal receipt must carry that exact ID.
