...
interface StructResolverExecutionControl {
  // Collision-resistant, supervisor-generated, and never reused during this supervisor lifetime.
...
type StructResolverExecutionHandle = {
  ...
  // One-shot: called only after Struct holds `control`; this is the sole authority-granting step.
...
The package measures the total budget with a monotonic clock and reserves `cleanupReserveMs` before allocation. `allocate()` may construct supervisor state but cannot start adapter code or grant credential/filesystem/network authority. Before returning control, the supervisor generates a collision-resistant execution ID and atomically reserves it in a lifetime registry containing both active IDs and immutable retired-ID tombstones. The ID must be unique across every active and retired execution and is never reused for the supervisor's lifetime. A concurrent collision or attempted retired-ID reuse fails allocation before adapter spawn or any credential/filesystem/network authority grant. The supervisor returns `control`, including idempotent `cleanup()`, before Struct invokes `start()`; every accepted revocation and terminal receipt must carry that exact reserved ID.

The supervisor enforces a one-shot state machine equivalent to `allocated -> starting -> running -> revoking -> cleaning -> terminated`, with failure transitions only toward revocation/cleanup/termination or parent fail-stop. `start()` atomically consumes the sole start right before any spawn or broker grant, succeeds at most once, and every duplicate call fails before spawning work or requesting capability. Once revocation begins, or after cleanup or termination begins, the start right is permanently closed. A late completion of a timed-out/revoked start is quarantined: it cannot publish a result, obtain or retain broker authority, spawn an accepted descendant, or return capacity to the pool.

`revokeAndQuarantine()` atomically and permanently closes future broker grants and descendant admission before it returns an accepted receipt. An accepted receipt requires `capabilitiesRevoked: true`, `quarantined: true`, and `executionId === control.executionId`, where the ID is still the active reservation for this exact handle; equality to a recycled or stale handle value is insufficient. `quarantined: false`, a mismatched ID, a revocation receipt swapped from another active or retired execution, throw, or missing/invalid receipt is revocation failure. Revocation failure never stops teardown: Struct still races iterator `return()` when available, calls pre-start `control.cleanup()`, and calls `terminateAndJoin()`. It then forces the separately supervised verification host to fail-stop, proves the host and descendants dead from the parent, and permanently retires the implicated executor capacity rather than reporting normal structured completion or reusing it.

Startup is inside the total budget and a throwing, rejecting, or never-settling start remains revocable and cleanable without a result object. Adapter code never runs synchronously in the caller's realm. At the streaming cutoff, or on success, synchronous startup throw, startup rejection/timeout, stream error, limit breach, or cancellation, Struct aborts, attempts synchronous revocation, races iterator `return()` when a result iterator exists, invokes and races pre-start `control.cleanup()` inside the reserved cleanup budget, and then calls and awaits `terminateAndJoin()` inside the reserved outer budget even when revocation, iterator return, or cleanup fails or times out. A normal terminal receipt must carry the active reservation for `control.executionId`; on valid termination the supervisor atomically converts that reservation to an immutable retired-ID tombstone before returning any capacity to the pool. A mismatch, a terminal receipt swapped from another active or retired execution, missing receipt, or never-settling terminal join forces the same parent-proven fail-stop, proves the implicated host and descendants dead, retires the implicated capacity, and records the ID's permanent retired tombstone. Any execution-ID collision discovered after authority grant receives that same fail-stop treatment rather than continuing under ambiguous identity. After accepted revocation, instrumentation must show zero broker grants/calls and zero admitted descendants for that execution.

The package checks protocol/profile version, resource ID, expected policy SHA-256, and the transport-kind-specific equality rules: HTTPS endpoint-map/endpoint/redirect IDs, filesystem root-capability ID, or content-addressed store ID, plus the bound execution receipts. The consumer owns the isolated executor, endpoint map, actual transport authority, descriptor root, DNS/private-address policy, and concurrency, but conformance requires one-shot start, lifetime-unique execution identity, revocation, cleanup, and fail-stop supervision to be independent of adapter cooperation and available before adapter startup. S-03 supplies the versioned conformance harness; every Ernie.SG or Aether executor must pass side-effect-free allocation, concurrent allocation attempts that return duplicate IDs, retired-ID reuse, duplicate start, start after revocation/cleanup/termination, synchronous startup throw, rejected and never-settling startup, late startup completion after timeout/revocation, clean-success join, quarantined-false receipt, revocation failure, mismatched revocation/terminal execution IDs, stale revocation/terminal receipts swapped between active or retired executions, zero post-revocation broker calls/grants/descendants, endpoint-map alias, secret URL/path/matrix/percent-encoding non-retention, DNS-rebinding, traversal/symlink, redirect-loop, deceptive-length, monotonic-budget, cancellation, pre-start cleanup after every startup outcome, iterator cleanup when available, never-settling cleanup, never-settling join in a sacrificial host, hard termination, residual-work, repeated-failure admission control, and aggregate-pressure tests. Duplicate or retired IDs must fail before authority grant; any identity ambiguity found after a grant must produce parent-proven fail-stop with implicated capacity retired. An executor that cannot return one-shot, independently revocable and cleanable supervisor control with a collision-resistant lifetime-unique execution ID before authority grant, or cannot produce execution-ID-bound revocation and terminal receipts outside adapter control, is not conforming.
...
E-ACT, A-REQ, and A-ACT are action-time authorization gates, not policy labels and not consequences of approving this ADR or creating its issue tree. Before accepting any gate receipt, the coordinator must authenticate and validate a new immutable explicit-user-authorization record for that gate. The record and accepted receipt both bind an authorization ID, immutable authorization-evidence SHA-256, authenticated authorizing-user identity, exact named action, exact target, authorized execution actor, issue time, expiry, and cost/billing scope. Cost scope is never omitted: it is either an explicit no-cost/no-billing statement or a currency, hard cap, billing owner, and permitted charge category. The receipt additionally binds the gate-specific exact head, artifacts, disclosures, thresholds, and recovery evidence. A missing, mutable, unauthenticated, stale, expired, action/target/actor/cost-mismatched, or replayed authorization is rejected before receipt acceptance and grants no downstream admission.

The three gates remain distinct. E-ACT authorizes only the named Ernie.SG writer/public-route activation at its immutable target. A-REQ authorizes only the named printer/contact/channel inquiry with its exact questions and disclosures. A-ACT authorizes one exact Aether route activation, deployment, or printer submission target; a different action or target needs a different authorization. The user's 2026-08-27 `go` authorized only persistence of the reviewed ADR/context and creation/linking of the 25 mapped issues. It is explicitly ineligible as authorization evidence for E-ACT, A-REQ, A-ACT, or any later implementation, publication, deployment, outreach, purchase, or billing action.
...
- Obtain a new immutable explicit-user-authorization record for the exact activation action, target, actor, expiry, and cost/billing scope; validate it before producing and accepting the target-specific activation receipt with its evidence digest, exact head, pins, migration/backfill result, validation evidence, rollout/observation thresholds, and rollback or forward-recovery constraints.
...
- Capture one real printer's requirements from user-supplied/public no-contact evidence or through an A-REQ inquiry backed by new immutable explicit-user authorization for the exact inquiry, target/contact, actor, expiry, disclosures, and cost/billing scope; validate the authorization before accepting the inquiry receipt. Then select the renderer/profile through a contract proof for the already selected A5 booklet plus three social cutdowns.
...
- No runtime activation occurs before Struct and Ernie.SG upstream gates pass, the selected A5 proof has a real printer/profile/renderer decision, and A-ACT has validated new immutable explicit-user authorization for the exact route/deployment or printer-submission action, target, actor, expiry, and cost/billing scope before accepting its receipt.
...
- E-ACT, A-REQ, and A-ACT each require separately authenticated immutable explicit-user-authorization evidence bound to the exact action, target, actor, expiry, and cost/billing scope and validated before receipt acceptance; prior ADR/issue-creation approval cannot satisfy any of them.
