...
description: '주말 프로토타입이 또 다른 실험으로 이어지며 배운 것들'
...
필요에 따라 파일 프로세서, 분할, 임베딩 및 파이프라인의 다른 단계에도 이 방식을 반복한다.
...
이 레지스트리와 추상 클래스 접근법은 다음 이점을 제공한다:

-   **확장성:** 딕셔너리(**`FILE_TYPE_PROCESSORS`**와 **`EMBEDDING_STRATEGIES`**)를 사용해 파일 유형과 임베딩 전략을 해당 프로세서에 쉽게 매핑한다. 새 파일 유형이나 임베딩 방법을 지원하는 일은 딕셔너리를 확장하는 것만큼 간단하므로 본질적으로 규모를 키우기 쉽다.
...
-   **유연성**: 매우 유연하다. 구성 요소가 딕셔너리에 등록되어 있으므로 교체하거나 새 구성 요소를 추가하는 일이 직관적이다. 예를 들어 ".py" 파일의 프로세서를 바꾸는 것은 **`FILE_TYPE_PROCESSORS`** 딕셔너리의 항목을 업데이트하는 것만큼 간단하다.
...
*원래 PubPub의 [erniesg.pubpub.org/pub/zc0zx741](https://erniesg.pubpub.org/pub/zc0zx741)에 게시됨.*
