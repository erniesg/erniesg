...
2.  **.json files で定義された Azure Data Factory リソース を 取り込み、分割する方法を理解する**
...
こうして 75 個の項目を `Deep Lake` のデータセットに分割して入れることになった。今はもう、さらに多くのデータソースを取り込み、すべての SQL スキーマや stored procedure、データそのものまで問い合わせられるようにすること、memory まわりのメタデータを更新すること、multi-modal data lake cum vector store 上で CRUD 操作を行うこと、検索方針と token context を最適化することまで考えている。そもそも、ツールを使って LLM の中で直接実行し、行動まで起こさせればいいのではないか。さらに言えば、LLM に自分でテストを書かせ、デバッグし、検証させればいい。これらはすべて、どんな規模のチームや組織でも控えめに言って生産性を 1000 倍にする autonomous agent を本番投入するうえで不可欠になる。
