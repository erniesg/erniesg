...
좌절해서 키보드를 두드리거나 [물려받은 codebase 앞에서 눈물을 조금 흘리는](https://www.activeloop.ai/resources/lang-chain-gpt-4-for-code-understanding-twitter-algorithm/) 대신, 그 정보를 A.I.에 넘기고 대화하는 편이 훨씬 말이 된다. 대형 언어 모델 (LLMs)의 지식과 추론 능력을 그런 정보 위에서 활용하기 시작하는 순간, 자연스러운 다음 단계는 synthesis다. 기존 codebase 위에서 실행될 수 있는 새 코드를 생성해, 전 세계 누구나 프롬프트-to-feature로 갈 수 있게 하는 것이다. 이런 과제를 다루기 위해 아래 demo를 만들었고, 주말 하루 이틀 정도 작업한 결과물이다. 만드는 과정에서 새로 배운 것도 많고 여러 개념을 더 깊이 이해하게 되어 꽤 재미있었다.
...
Solution: 알고 보니 특정한 `streamlit (1.25.0)`과 `pydanic version (1.10.12)`를 써야 했다. 여러 packages 사이의 compatibility issue가 꽤 자주 반복되니, 이를 관리하는 도구와 모범 사례를 더 배워야 할 것 같다. 어쩌면 사용한 구체 버전은 항상 `poetry` 안에 정의되어야 하는 것 아닐까!?

2.  **.json 파일로 정의된 Azure Data Factory 리소스를 ingest하고 chunk하는 방법 알아내기**
    

문서, .PDFs, GitHub 코드 저장소 등을 처리하는 튜토리얼은 많지만 .json 언급은 거의 없었다. ChatGPT에게 생성해달라고 해도 소용이 없어서, 결국 .json과 `langchain` JSON loader를 자세히 들여다보고 내가 생각하는 방식대로 단계별로 ChatGPT를 안내했더니 마침내 작동했다. 적어도 Google과 YouTube 검색 첫 페이지 기준으로 Azure Resource Manager (ARM) template에 대해 이 작업을 한 사람은 없었으니, 조금은 새로움을 만들어야 했다.

Solution: 이 초기 프로토타입에서는 Azure Data Factory의 `parameters`, `variables`, `resources`와 `resources` 안의 individual objects를 보존하고 싶었다. 그것들이 내 pipelines, activities 등이 될 것이기 때문이다. 여기에 few shot instructions와 맥락를 결합하자 ChatGPT가 필요한 결과를 만들어냈다.

# 앞으로의 방향

그렇게 75개 items를 `Deep Lake` 데이터set으로 chunk하게 되었고, 이제 더 많은 데이터 소스s를 ingest할 필요를 생각하고 있다. 모든 SQL schemas, stored procedures, 데이터 itself까지 query하게 해보자! memory와 관련해 meta데이터를 업데이트하고, multi-modal 데이터 lake cum vector store 위에서 CRUD 작업를 수행하고, 검색 strategies와 token 맥락s를 최적화하는 일도 있다. 사실 도구를 사용해 LLM 안에서 직접 실행하고 action을 취하면 안 될 이유가 있을까? 더 나아가 LLM이 스스로 tests를 작성하고, 디버그하고, validate하게 만들면 어떨까? 이 모든 것은 어떤 규모의 팀이나 조직이든 보수적으로 말해 1000x productivity를 가능하게 할 autonomous 에이전트를 프로덕션에 올리는 데 필수적일 것이다.
