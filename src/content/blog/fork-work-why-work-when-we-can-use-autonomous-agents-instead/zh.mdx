...
2.  **弄清楚如何 ingest 和 chunk 用 .json 文件定义的 Azure Data Factory 资源**
...
这就是我如何最终把 75 个 items chunk 到我的 `Deep Lake` 数据set 里。现在我已经在思考 ingest 更多 数据源s 的需求（让我们查询所有 SQL schemas、stored procedures 和 数据 itself！）、围绕 memory 更新 meta数据、对 multi-modal 数据 lake cum vector store 执行 CRUD 操作、优化 检索 strategies 和 token 上下文s。事实上，为什么不直接让 LLM 通过 工具 执行并采取行动？更进一步，为什么不让 LLM 写自己的 tests、调试 并 validate 自己？所有这些对于把一个 autonomous 智能体 投入生产都很关键，它将让任何规模的团队或组织保守估计也能实现 1000x productivity。
