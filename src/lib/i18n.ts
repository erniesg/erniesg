...
export const TAG_LABELS: Record<SupportedLocale, Record<string, string>> = {
  en: {
    ...
    content: 'Content',
    'developer diaries': 'Developer diaries',
    ...
    'sinking markets': 'Sinking markets',
    startups: 'Startups',
    streaming: 'Streaming',
    tools: 'Tools',
    vercel: 'Vercel',
  ...
  ko: {
    ...
    architecture: '아키텍처',
    astro: 'Astro',
    'build in public': '공개 개발',
    cloudflare: 'Cloudflare',
    content: '콘텐츠',
    'developer diaries': '개발자 일지',
    engineering: '엔지니어링',
    grief: '슬픔',
    hosting: '호스팅',
    investment: '투자',
...
> = {
  en: {
    ...
    'author.erniesg.name': 'Chen Enjiao (Ernie)',
    'about.title': 'About',
    'about.copy':
...
export function getAuthorDisplayName(
  ...
  const key = `author.${authorId}.name`
  const translatedName = getStaticTranslation(key, locale)
  return translatedName === key ? fallbackName : translatedName
}
