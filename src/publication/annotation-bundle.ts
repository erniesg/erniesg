...
export function createAnnotationBundle(
  ...
  for (const entry of anchors) {
    if (!nodeIds.has(entry.anchor.nodeId)) {
      throw new Error(
        `Anchor ${entry.id} targets missing graph node ${entry.anchor.nodeId}`,
      )
    }
  }
  for (const annotation of annotations) {
    if (!nodeIds.has(annotation.target.nodeId)) {
      throw new Error(
        `Annotation ${annotation.id} targets missing graph node ${annotation.target.nodeId}`,
      )
    }
  }
