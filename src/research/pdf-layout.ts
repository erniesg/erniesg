...
function hasStrongCitationSurfaceOwnership(
        ...
        .filter(
          (surface) =>
            ...
            surface.sourceBoxes.some((box) =>
              hasStrongCitationSurfaceOwnership(annotationBox, box),
            ) &&
            surface.start >= 0 &&
...
export function resolveCanonicalHyperlinkObligations({
  ...
  for (let leftIndex = 0; leftIndex < annotations.length; leftIndex += 1) {
    ...
    for (
      ...
      if (right.page !== left.page) continue
  ...
  for (const annotation of annotations) {
    ...
    if (annotation.status === 'unresolved') {
      ...
      const referenceSurfaceBlocks = isReferenceDestination
        ? [
            ...new Set(
              canonicalInternalSurfaces.flatMap((surface) =>
                surface.sourceBoxes.some((box) =>
                  hasStrongCitationSurfaceOwnership(annotation.box, box),
                )
      ...
      const annotationBlocks = [
        ...new Set([...sourceOwnedBlocks, ...referenceSurfaceBlocks]),
      ]
