...
import {
  calculateProvenanceTextConservation,
  type ProvenanceTextConservationInput,
} from './pdf-quality-provenance'
import { auditSourceSemanticFlowBoundaryLedger } from './pdf-quality-semantic-flow'
export { sourceSemanticFlowHyphenVerdict } from './pdf-quality-semantic-flow'
...
export function provenanceTextConservation(
  input: ProvenanceTextConservationInput,
) {
  return calculateProvenanceTextConservation(input, {
    auditSourceSemanticFlowBoundaryLedger: (auditInput) =>
      auditSourceSemanticFlowBoundaryLedger(auditInput, {
        rtlBaseDirection,
        sentenceEndWithClosing: PDF_SENTENCE_END_WITH_CLOSING,
        sameSourceBox,
        isPdfSourceSemanticFlowBoundaryDecision,
      }),
    nodeText,
    validatedVisualRepresentationByNode,
    unresolvedAuthorPlaceholder: UNRESOLVED_AUTHOR_PLACEHOLDER,
  })
}
