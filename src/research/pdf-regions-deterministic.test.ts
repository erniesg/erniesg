...
import { classifyPdfFurniture as classifyPdfFurnitureDirect } from './pdf-furniture'
...
it('keeps the page-furniture classifier directly importable', () => {
  const result = classifyPdfFurnitureDirect([
    page(1, [run(1, 'Journal of Source Evidence', 0.12, 0.03, 0.35, 8)]),
    page(2, [run(2, 'Journal of Source Evidence', 0.12, 0.03, 0.35, 8)]),
  ])
  ...
  expect(result.patterns).toMatchObject([
    {
      ...
      band: 'top',
      pages: [1, 2],
      normalizedText: 'journal of source evidence',
    },
  ])
})
