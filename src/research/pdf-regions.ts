...
function digitValue(character: string) {
  ...
  if (codePoint < 0 || !/^\p{Nd}$/u.test(String.fromCodePoint(codePoint))) {
    return null
  }

  // Decimal digit sets are ten code points wide, although some scripts place
  // two or more sets next to one another (and mathematical digits place five
  // sets in one contiguous run). Find the containing run and reset at each
  // group of ten so newly assigned Unicode ranges remain covered without a
  // stale hand-maintained range table.
  let runStart = codePoint
  while (runStart > 0 && /^\p{Nd}$/u.test(String.fromCodePoint(runStart - 1))) {
    runStart -= 1
  }
...
export function reconstructPageRegions(
  ...
  const lineFurniture = (line: PdfTextLine) => {
    if (protectedRepeatedMarginHeadings.has(line)) return undefined
    const visibleRuns = line.runs.filter((run) => run.text.trim())
