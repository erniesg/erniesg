# 1. Introduction: The Politics of Transnational Chinese Correspondence

This research explores the intersection of cultural heritage, interactive design, and embodied experience to transform static historical Chinese diaspora correspondence into dynamic, living encounters. Focusing on _qiao pi_ (侨批) or “letters home” held in the National Museum of Singapore’s collection, we address a fundamental challenge in cultural heritage presentation: how to make historical correspondence emotionally resonant with contemporary audiences while challenging dominant interpretive frameworks.

Chinese _qiao pi_—remittance letters sent by overseas Chinese to their families from the 19th to mid-20th centuries—were recognised by UNESCO as Memory of the World in 2013, acknowledging their significance as crucial lifelines connecting diaspora communities with their families in China (UNESCO, 2013). Despite this recognition, a comprehensive review of existing research reveals significant gaps, particularly regarding marginalised voices and innovative presentation methods.

## 1.1 Dominant Narratives: Nationalism and Historical Contextualisation

A significant body of scholarship positions _qiao pi_ within nationalist frameworks that emphasise patriotic loyalty to the Chinese motherland. This instrumentalisation is evident in both academic literature and museum presentations, creating what Wang (2019) describes as patriotic educational tools through the display of _qiao pi_ archives.[^1]

This state-sanctioned narrative prioritises content demonstrating how “overseas Chinese elevated their love for family, clan, and hometown to love for their motherland” (Bai, 2018, p. 131).[^2] Scholars like Ye (2022) explicitly position _qiao pi_ as materials for promoting “traditional Chinese virtues such as patriotism” and conducting “patriotic education.”[^3] Similarly, Lin and Huang (2020) document how _qiao pi_ are used in education to cultivate “patriotic sentiments” through historical stories related to “loving the party and country.”[^4]

Historical contextualisation studies frequently reinforce these nationalist frameworks. Zheng (2019) frames _qiao pi_ as evidence of how overseas Chinese “threw themselves into revolutionary activities and anti-Japanese resistance movements.”[^5] Chen and Cen (2021) analyse patriotic sentiments in family correspondence during the war against Japan, showing how resistance to Japanese aggression strengthened cultural identification with China. This historical contextualisation extends to earlier periods as well, with Bai (2018) documenting overseas Chinese involvement in the 1911 Revolution.

While patriotic sentiments may indeed appear in some correspondences, particularly during periods of national crisis, this dominant interpretive framework risks creating a monolithic narrative that filters overseas Chinese experiences primarily through the lens of state interests. The focus on nationalist themes overshadows the transnational nature of these communications, where identities were negotiated across borders rather than simply affirming singular national allegiances.

## 1.2 Institutional and Economic Networks

Scholarship examining the institutional infrastructure of _qiao pi_ focuses primarily on remittance houses (_qiao pi ju_) and their transnational banking networks. Dai (2003) pioneered research on these networked enterprises, analysing how they operated within embedded social structures, while his subsequent work examined the transition between traditional and modern business systems as _qiao pi ju_ developed relationships with formal banking institutions (Dai, 2004). Building on this foundation, Jiao (2010) explored the construction and operation of transnational commercial networks, particularly in Fujian.

Western scholarship has similarly recognised these sophisticated financial mechanisms. Harris (2013) analysed “the organisational structure and business practices of Qiaopiju” (p. 130), demonstrating how these firms adapted to emerging regulatory regimes across borders. Benton and Liu (2018) dedicate entire chapters in their book _Dear China_ to examining “The Structure of Qiaopi Trade and Transnational Networks” and “The Qiaopi Trade as a Distinctive Form of Chinese Capitalism.” Their work represents the first book-length study in English of the origins, structure, and operations of the _qiao pi_ trade.

## 1.3 Cultural and Social Dimensions

### 1.3.1 Traditional Values and Cultural Analysis

A significant strand of research examines how _qiao pi_ embodied traditional Chinese values, particularly concepts of trust and familial responsibility that sustained remittance networks without formal legal protections. Niu (2013) demonstrates how traditional Confucian moral principles of “benevolence, righteousness, propriety, wisdom, and trustworthiness”[^6] permeate these archives, extending their significance beyond mere economic transactions. Li et al. (2023) further this analysis by examining how “trust culture”[^7] in Guangdong _qiao pi_ functioned across personal, industry, and national levels. These studies consistently highlight how Confucian values of filial piety motivated the maintenance of transnational connections (Deng, 2009).

### 1.3.2 Gender Imbalances and Male-Centric Perspectives

Research on _qiao pi_ largely reflects historical gender imbalances in overseas Chinese communities. Tian and Xiao (2015) examines letters between male family members, while Zhou (2020) quantifies this gender disparity, documenting that Singapore’s 1931 Hainanese community had a gender ratio of 1000:151 (male:female). This predominant focus on male perspectives has marginalised women’s voices and experiences in both the historical record and subsequent scholarly analysis.

## 1.4 The Critical Gap: Marginalised Hainanese Women’s Voices

Given the extensive coverage discussed above, this research contributes to addressing gaps in the landscape by:

- Deconstructing monolithic narratives of Chinese identity by including diverse voices, particularly those of Hainanese women

- Prototyping innovative museum practices that enhance visitor engagement with textual artifacts

- Advancing theoretical understandings of embodied heritage engagement

Singapore’s position as a multicultural society with significant Chinese diaspora populations offers a critical alternative to the PRC’s state-centered interpretations. As Frost (2003) argues, Singapore’s approach to Chinese heritage necessarily differs from mainland narratives, creating space for what Ang (2001) describes as the “impossible position” of diaspora communities—caught between essentialist notions of Chinese identity and their own complex, hybrid experiences. This research leverages Singapore’s unique cultural positioning to develop counter-narratives that deconstruct monolithic conceptions of “Chineseness” while creating new nexuses of connection.

## 1.5 Multiple Layers of Marginalisation

The National Museum of Singapore operates within a multicultural national narrative that positions Chinese heritage as one component of a diverse society rather than as an extension of mainland Chinese nationhood. Such institutional positioning creates space for what Yeoh and Kong (2012) describe as “multivocal heritage interpretations” that recognise diverse perspectives within and across ethnic categories.

Within this context, Hainanese women’s correspondence represents voices subject to multiple layers of marginalisation. The Hainanese themselves were marginalised within broader Chinese society long before migration. Kiungchow (modern-day Haikou) was historically “an outpost where for over a thousand years Chinese officials were banished for offending their emperor” (Nield, 2015, p. 141). This profound geographical marginalisation shaped the island’s cultural and economic development, positioning it as peripheral to mainstream Chinese society.

In Singapore, the Hainanese faced additional layers of marginalisation. Arriving later than other dialect groups, they constituted only the fifth-largest Chinese dialect group (Conceicao, 2023). Their linguistic differences made communication with other Chinese communities difficult, as their dialect was largely unintelligible to Hokkien, Teochew, and Cantonese speakers. Economically marginalised, they were forced into service sectors like cooking and domestic work as more lucrative trades were already dominated by earlier Chinese migrants.

Hainanese women thus experienced at least three intersecting layers of marginalisation: as residents of a peripheral island historically used for political exile; as late-arriving migrants to Singapore with linguistic barriers; and as women within patriarchal structures that viewed female migration with suspicion. Their correspondence represents what Bhabha (1994) would describe as “border lives” at the intersection of multiple forms of marginality—experiences that profoundly challenge simplistic nationalist narratives centered on patriotic loyalty to the motherland.

## 1.6 The Challenge of Making Letters Accessible

Beyond the political dimensions of interpretation, Chinese diaspora correspondence presents significant practical challenges for contemporary museum display. As Serrell (2015) discusses, lengthy exhibit labels can lead to reduced visitor engagement in museum settings. This engagement challenge is particularly acute for materials written in classical Chinese or regional dialects, which remain inaccessible to many visitors including younger Singaporeans.

Traditional approaches to presenting historical correspondence—through translated displays, video recreations, or touchscreen interactives—frequently fail to create meaningful engagement with these materials. As Smith (2006) argues, heritage interpretations often impose fixed narratives that privilege curatorial authority over visitor meaning-making. Digital interfaces, while providing information access, often create what Parry (2010) describes as “informational rather than affective engagement” (p. 123)—privileging factual knowledge over emotional connection.

This recognition has led to our exploration of biometric approaches to visitor engagement—specifically, heart rate synchronisation as a means of creating embodied connections with historical materials. By translating the emotional content of letters into sound experiences that respond to visitors’ heart rates, we address both the political challenge of challenging nationalist narratives and the practical challenge of making these materials accessible to diverse visitors. This represents a first-of-its-kind presentation of _qiao pi_ in biometric installations.

# 2. New Museology and Embodied Experience Applied

Our approach integrates new museology and embodied experience to create a coherent framework for understanding and engaging with marginalised diaspora correspondence. These interconnected perspectives inform both our analysis of letter content and our development of biometric engagement methods.

As articulated by Hooper-Greenhill (2000), new museology questions authoritative curatorial voices and seeks more democratic, visitor-centred, and multivocal presentations of cultural artefacts. This theoretical framework positions museums not simply as repositories of objects but as sites of experience, meaning-making, and social engagement. Simon’s (2010) concept of the “participatory museum” also provides a particularly relevant framework for this research.

Complementary to this is Gurian’s (2006) concept of “essential museums,” which emphasises that objects serve primarily as conduits for human connection and storytelling rather than as ends in themselves. This perspective is particularly valuable for text-heavy artefacts like letters, which derive their significance not from their material properties but from the human experiences and relationships they embody.

The biometric application is also grounded in emerging scholarship on embodied knowledge and affective history. Crouch (2003) argues that bodily engagement transforms abstract knowledge into lived experience, creating deeper and more meaningful connections to cultural content. Similarly, Witcomb (2013) demonstrates how affective engagement in museum spaces can create stronger connections to historical materials than purely cognitive approaches. By synchronising visitors’ heart rates with soundscapes derived from letter content, the installation creates emotional and bodily connections that collapse temporal distance and foreground shared human experiences across historical contexts, aligning with Ahmed’s (2014) exploration of how emotions can bridge temporal and cultural divides. Tröndle and Tschacher’s (2012) research on “embodied resonance” between visitors and cultural artefacts supports our approach to creating affective connections through biometric synchronisation.

This integrated framework supports a multi-sensorial approach to heritage engagement where visitors “feel” their way into historical experiences that might otherwise remain inaccessible. By synchronising traditional percussion patterns to visitors’ heart rates, we create what Witcomb (2015) describes as a “pedagogy of feeling”—learning through emotional and bodily engagement rather than purely intellectual analysis. This approach addresses what she identifies as the challenge of creating “affective histories” that engage emotional and bodily responses alongside intellectual understanding.

# 3. Methodology: From Content to Experience - Letters, Heartbeats, and Sound

Having reviewed the corpus of relevant existing studies and areas in which this paper aims to contribute, the following section will detail how we arrive at the chosen letters, discuss further the value of heart rate synchronisation as well as the final prototype developed.

## 3.1 Letter Selection

The selection of _qiao pi_ for analysis followed a multi-stage filtering process. Beginning with 2,265 items from the National Museum of Singapore’s collection containing the keyword “letter,” we excluded non-paper materials to reach 887 items. Further manual review identified 385 Chinese letters, from which we focused particularly on correspondence involving Hainanese women.

Our focus on Hainanese women’s letters was both incidental and deliberate. Incidental because these were among the few letters in the collection that featured a female sender or recipient, and deliberate in addressing significant gaps in existing research which has predominantly examined male-authored correspondence. It’s worth noting that some letters may have been written by professional letter writers rather than the stated senders themselves, as literacy rates were low, particularly among women. However, even professionally written letters would typically capture the sender’s intended messages and concerns, filtered though they might be through the scribe’s writing conventions.

Our analysis of Hainanese women’s letters reveals themes and concerns that are more quotidian than nationalistic. Two specific letters exemplify these counter-narratives:

### Letter 1 · He Aifang to He Qinge

Letter from He Aifang (何爱芳) to He Qinge (何庆娥) in Haikou, Hainan, dated March 2, 1960. National Museum of Singapore, Accession No. 2000-06963-004, “Remittance letters delivered by Shun Chen Rong Remittance House.”

<figure class="archive-letter archive-letter--portrait">
  <img src="/research/if-letters-home-could-sing/media/image3.jpg" alt="Historical remittance letter from He Aifang to He Qinge, handwritten vertically in Chinese" />
  <figcaption>Letter 1. From He Aifang to He Qinge (March 2, 1960). Source: National Museum of Singapore, Accession No. 2000-06963-004.</figcaption>
</figure>

The letter acknowledges receipt of Hong Kong dollars and discusses the education of siblings—content that centres family relationships and educational planning rather than national allegiance.

### Letter 2 · A mother in Hainan to Fu Zhifeng

Letter from a mother in Hainan to her son Fu Zhifeng (符致逢), dated March 10, 1954. National Museum of Singapore, Accession No. 2000-06965-002, “Letter to Fu Zhi Fong from mother in Hainan.”

<figure class="archive-letter">
  <img src="/research/if-letters-home-could-sing/media/image2.jpg" alt="Historical remittance letter from a mother in Hainan to Fu Zhifeng, handwritten vertically in Chinese" />
  <figcaption>Letter 2. From a mother in Hainan to Fu Zhifeng (March 10, 1954). Source: National Museum of Singapore, Accession No. 2000-06965-002.</figcaption>
</figure>

The letter mentions receipt of Hong Kong dollars, discusses the mother’s health concerns, and requests a visit with tonics. The content centres intimate family relationships, bodily concerns, and practical matters rather than national or political themes.

These letters reveal what Scott (1990) describes as “hidden transcripts”—perspectives and experiences that exist outside officially sanctioned narratives. These materials document everyday concerns, family relationships, and personal struggles that complicate nationalist framings emphasising patriotic loyalty. By centring these marginalised voices, we contribute to what Chakrabarty (2000) describes as “provincializing” dominant historical frameworks.

The ordinary lives documented in Hainanese women’s letters hold profound significance for world history. As Hsu (2000) argues, diaspora correspondence provides “history from below” that reveals how global forces—colonialism, capitalism, and nation-building—were experienced, negotiated, and sometimes resisted by ordinary people. The use of Hong Kong dollars reflects how these women navigated complex currency regimes and financial networks established by colonial powers. Their concerns about health, education, and family well-being demonstrate how care economies functioned across transnational spaces, maintaining social cohesion despite geographic separation.

## 3.2 The Universality of Heartbeats

The heart rate provides a particularly appropriate biometric signal for connecting visitors to historical materials for several reasons:

1.  **Truly Universal**: Everyone has a heartbeat, creating a genuinely universal entry point to cultural materials that transcends linguistic, cultural, and educational differences.

2.  **Embodied Significance**: The heartbeat carries powerful metaphorical associations with life, emotion, and human connection across most cultural traditions, making it an intuitive connector to historical materials about human relationships. Indeed, the heart is often metaphorically associated with emotions across cultures (Kövecses, 2000). The phrase “breathing life” into history becomes literal through this physiological connection.

3.  **Below Conscious Control**: Heart rate operates both below conscious control and within awareness, representing a form of what Nacke et al. (2011) term “implicit interaction” (p. 103)—physiological engagement that occurs without deliberate user actions. This characteristic makes heart rate potentially valuable for museum contexts where visitors may feel self-conscious about explicit interactive behaviors (Hornecker & Stifter, 2006, p. 138).

4.  **Biofeedback Loop**: This setup, where heart rate influences the soundscape and the soundscape may in turn affect the visitor’s physiological state, exemplifies a biofeedback loop, a concept well-established in physiological computing.

By using heart rate as the physiological bridge between visitor and historical material, we allow visitors to literally “feel” their way into someone else’s story, creating a multi-sensorial pathway to understanding that doesn’t require the ability to read classical Chinese.

## 3.3 Interactive Prototype: Heart-Synchronised Percussion

Our interactive prototype translates the emotional content of Hainanese women’s letters into sound experiences synchronised with visitors’ heart rates. The system architecture comprises primary components as detailed below:

<figure class="system-diagram">
  <div class="letters-system-diagram">
    <svg viewBox="0 0 360 820" role="img" aria-labelledby="letters-diagram-title letters-diagram-description">
      <title id="letters-diagram-title">Heart-synchronised letter installation architecture</title>
      <desc id="letters-diagram-description">A visitor's heart-rate monitor and letter selection feed an audio engine. The engine analyses heart rate and emotion, maps tempo, selects base patterns, generates a three-part musical sequence, and sends it to both the sound and visual systems.</desc>
      <defs>
        <marker id="letters-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
          <path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor"></path>
        </marker>
      </defs>
      <rect class="diagram-group" x="12" y="12" width="336" height="150" rx="8"></rect>
      <text x="28" y="38" font-size="13" font-weight="600">Visitor input</text>
      <rect class="diagram-node" x="28" y="58" width="136" height="38" rx="4"></rect>
      <text x="96" y="82" font-size="12" text-anchor="middle">Heart-rate monitor</text>
      <rect class="diagram-node" x="196" y="58" width="136" height="38" rx="4"></rect>
      <text x="264" y="82" font-size="12" text-anchor="middle">BLE connection</text>
      <path class="diagram-line" d="M164 77 H196" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-node" x="28" y="110" width="136" height="38" rx="4"></rect>
      <text x="96" y="134" font-size="12" text-anchor="middle">Letter selection</text>
      <path class="diagram-line" d="M264 96 V188 H238" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M96 148 V188 H122" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-group" x="12" y="178" width="336" height="430" rx="8"></rect>
      <text x="28" y="204" font-size="13" font-weight="600">Audio engine</text>
      <rect class="diagram-node" x="188" y="220" width="144" height="40" rx="4"></rect>
      <text x="260" y="245" font-size="12" text-anchor="middle">Heart-rate processor</text>
      <rect class="diagram-node" x="28" y="220" width="136" height="40" rx="4"></rect>
      <text x="96" y="245" font-size="12" text-anchor="middle">Emotion analysis</text>
      <rect class="diagram-node" x="188" y="292" width="144" height="40" rx="4"></rect>
      <text x="260" y="317" font-size="12" text-anchor="middle">Tempo mapping</text>
      <rect class="diagram-node" x="28" y="292" width="136" height="40" rx="4"></rect>
      <text x="96" y="317" font-size="12" text-anchor="middle">Base-pattern selection</text>
      <path class="diagram-line" d="M260 260 V292" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M96 260 V292" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-node" x="86" y="364" width="188" height="42" rx="4"></rect>
      <text x="180" y="390" font-size="12" text-anchor="middle">Pattern generation</text>
      <path class="diagram-line" d="M260 332 V348 H196 V364" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M96 332 V348 H164 V364" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-node" x="86" y="438" width="188" height="42" rx="4"></rect>
      <text x="180" y="464" font-size="12" text-anchor="middle">Section manager</text>
      <path class="diagram-line" d="M180 406 V438" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-node" x="28" y="516" width="88" height="50" rx="4"></rect>
      <text x="72" y="537" font-size="11" text-anchor="middle">Entrance</text>
      <text x="72" y="552" font-size="10" text-anchor="middle">patterns</text>
      <rect class="diagram-node" x="136" y="516" width="88" height="50" rx="4"></rect>
      <text x="180" y="537" font-size="11" text-anchor="middle">Emotional</text>
      <text x="180" y="552" font-size="10" text-anchor="middle">patterns</text>
      <rect class="diagram-node" x="244" y="516" width="88" height="50" rx="4"></rect>
      <text x="288" y="537" font-size="11" text-anchor="middle">Closing</text>
      <text x="288" y="552" font-size="10" text-anchor="middle">patterns</text>
      <path class="diagram-line" d="M180 480 V498 H72 V516" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M180 480 V516" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M180 480 V498 H288 V516" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M72 566 V636 H96" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M180 566 V620 H180 V636" marker-end="url(#letters-arrow)"></path>
      <path class="diagram-line" d="M288 566 V636 H264" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-group" x="12" y="626" width="158" height="132" rx="8"></rect>
      <text x="28" y="652" font-size="13" font-weight="600">Sound system</text>
      <rect class="diagram-node" x="28" y="670" width="126" height="34" rx="4"></rect>
      <text x="91" y="692" font-size="11" text-anchor="middle">Sample selection</text>
      <rect class="diagram-node" x="28" y="714" width="126" height="30" rx="4"></rect>
      <text x="91" y="734" font-size="11" text-anchor="middle">Audio output</text>
      <path class="diagram-line" d="M91 704 V714" marker-end="url(#letters-arrow)"></path>
      <rect class="diagram-group" x="190" y="626" width="158" height="132" rx="8"></rect>
      <text x="206" y="652" font-size="13" font-weight="600">Visual system</text>
      <rect class="diagram-node" x="206" y="670" width="126" height="34" rx="4"></rect>
      <text x="269" y="692" font-size="11" text-anchor="middle">Letter reveal</text>
      <rect class="diagram-node" x="206" y="714" width="126" height="30" rx="4"></rect>
      <text x="269" y="734" font-size="11" text-anchor="middle">Pulse visualisation</text>
      <path class="diagram-line" d="M269 704 V714" marker-end="url(#letters-arrow)"></path>
    </svg>
  </div>
  <figcaption>Figure 1. System diagram. Created by the author.</figcaption>
</figure>

5.  **Heart Rate Integration**: The system takes real-time heart rate data from a Bluetooth Low Energy (BLE) device, using a mapping function that converts physiological heart rates (60-120 BPM) to musical tempos (65-95 BPM). This creates a direct connection between the visitor’s bodily state and the musical experience.

6.  **Letter Emotional Analysis**: Each letter has predefined emotional metrics capturing its content, including emotional intensity, urgency, and dominant emotions. These metrics are based on close textual analysis of the letters. The emotional analysis component translates these textual emotional dimensions into parameters for sound generation, facilitating a cross-modal mapping from text to sound, a process central to the field of affective computing (Picard, 1997).

7.  **Musical Pattern Generation**: The system generates percussion patterns structured in three sections (entrance, emotional, and exit), creating a complete narrative arc for each letter. The patterns use traditional opera percussion instruments—Bangu (板鼓) as the main rhythmic instrument, Daluo (大锣) for accent points, Xiaoluo (小锣) for ornamental flourishes, and Naobo (铙钹) for punctuation . For the purpose of quick prototyping, the percussions were taken from this dataset of Beijing opera as it was readily available (Tian, Srinivasamurthy, Sandler, & Serra, 2014). This approach creates what Kirshenblatt-Gimblett (1998) describes as “cultural resonance”—connections between historical materials and their living cultural contexts. The interactive experience follows a structured flow with three distinct phases:

    - **Entrance Section**: The experience begins with the gradual fade-in of visuals and simple rhythmic patterns as the visitor’s heart rate visualisation begins. This phase serves as a “scaffolded entry” that allows visitors to understand how their heart rate influences the experience before full engagement.

    - **Emotional Section**: The core experience features the full letter reveal, complex pattern variations, and peak emotional expression. During this phase, the visitor’s heart rate directly influences the tempo and intensity of the percussion patterns, creating an “affective resonance” between visitor and historical material.

    - **Exit Section**: The experience concludes with pattern simplification, visual fade-out, and gradual tempo reduction. This phase provides closure and reflection, embodying what Simon (2010) describes as the “me-to-we” design principle—moving from personal experience to broader social and historical connections.

This system creates a dynamic, responsive experience where the user’s heart rate influences both the musical and visual elements, while the emotional content of the letters provides the structural framework for the experience. The three-section structure (entrance, emotional, exit) ensures a complete narrative arc, while the real-time heart rate input keeps the experience personal and engaging.

# 4. Implications

This project demonstrates the practical application of key concepts from new museology. Its relevance to museum practice, in proposing counter-narratives and making visible marginalised voices will be discussed below.

## 4.1 Museum Practice Transformation

The findings from this research have significant implications for museum practice, particularly for the display and interpretation of text-heavy historical artefacts. Traditional approaches to exhibiting letters—displaying them under glass with accompanying explanatory text—can create significant barriers to engagement, particularly for items in languages unfamiliar to most visitors (Bitgood, 2013). The biometric approach demonstrates a viable alternative that can significantly increase both engagement duration and emotional connection with such materials.

The biometric approach also offers practical solutions to engaging visitors across Singapore’s linguistically diverse population. By creating non-linguistic points of access to these materials, the installation enables meaningful engagement regardless of visitors’ fluency in Chinese dialects or Chinese writing, making these important historical materials accessible to all Singaporeans regardless of linguistic background.

## 4.2 Countering Nationalist Mono-Narratives

For heritage institutions, particularly those in contexts like Singapore with complex relationships to larger nation-states like China, this research demonstrates practical approaches to developing counter-narratives that challenge dominant nationalist framings. By highlighting everyday concerns, women’s voices, and local contexts in the presentation of diaspora materials, museums can create more nuanced understandings of transnational experiences that resist subsumption under singular national narratives.

This approach has particular relevance in contemporary contexts where heritage increasingly functions as a domain of soft power and cultural diplomacy. As Winter (2015) notes in his analysis of heritage diplomacy in Asia, cultural institutions often navigate complex geopolitical dynamics in their presentation of shared heritage. Singapore’s presentation of Chinese diaspora materials in ways that acknowledge connections to China while emphasising distinctive local developments represents an important model for balancing these considerations.

## 4.3 Amplifying Access to Marginalised Voices

Perhaps most significantly, this research offers practical mechanisms for amplifying historically marginalised voices. By making women’s letters—often overlooked in traditional scholarship—accessible and emotionally resonant for contemporary audiences, the project demonstrates how technological innovation can serve broader goals of inclusivity and diverse representation in cultural heritage. For the National Museum of Singapore specifically, this approach aligns with institutional commitments to representing the nation’s multicultural heritage and complex historical relationships.

# 5. Conclusion and Future Research

In summary, “If Letters Home Could Sing” demonstrates how embodied heritage engagement can challenge nationalist narratives while making historically significant materials accessible to diverse audiences. For Singapore, this project represents an opportunity to deconstruct monolithic narratives of Chinese identity by including diverse voices, creating an alternative nexus of story that binds people together through shared human experience rather than national allegiance. By centring Hainanese women’s voices and developing innovative approaches to their presentation, Singapore’s heritage institutions can challenge the mono-narrative of patriotic loyalty to the motherland while affirming the global significance of ordinary lives that traversed national boundaries.

The methodology developed through this research demonstrates how embodied, physiological engagement can create meaningful connections with historically and linguistically complex materials. By literally allowing visitors to “breathe life” into historical letters through their heartbeats, we create possibilities for understanding the past not as a distant, fixed narrative but as a living presence that continues to resonate through our bodies and communities.

Future directions for this research include expanding the methodology to other marginalised correspondence in museum collections, developing more sophisticated emotional mapping techniques, and conducting longitudinal studies of visitor engagement and learning outcomes. The prototype can also be developed further with more informed musical involvement and clan institutions.

As museums globally seek ways to remain relevant in an increasingly digital world, “If Letters Home Could Sing” offers a model that transcends the binary of technological innovation versus human experience. Indeed, this research suggests a way forward where museums become spaces not just for preserving the past, but for creating living connections between cultural heritage and contemporary experience – one heartbeat at a time.

# Acknowledgments

This research was supported by the National Heritage Board through the National Museum of Singapore’s 2024 Research Fellowship programme. I extend my sincere gratitude to the curatorial team at the National Museum of Singapore for their invaluable guidance and for providing access to their rich collection of _qiao pi_ materials.

> **Disclaimer.** The opinions, findings, and recommendations expressed in this publication are those of the Research Fellow and do not necessarily reflect the views of the National Heritage Board.

On a personal note, this research is deeply inspired by and dedicated to the incredible Hainanese women in my life—my late mother and grandmother—whose strength, resilience, and wisdom continue to influence me. It feels particularly meaningful that the letters I found most compelling were written by Hainanese women; I did not set out specifically to find these voices, but perhaps they found their way to me. By amplifying these historically marginalised voices, I hope to honour my family heritage while revealing how women's everyday concerns formed the essential yet often unacknowledged foundation upon which strong, transnational communities were built and sustained.

# References

Ahmed, S. (2014). _The cultural politics of emotion_ (2nd ed.). Edinburgh University Press.

Ang, I. (2001). _On not speaking Chinese: Living between Asia and the West_. Routledge.

Bai, J. \[白婧\]. (2018). 黄清海《菲华黄开物侨批：世界记忆财富（1907–1922）》\[Huang Qinghai “Philippine Chinese Huang Kaiwu qiao pi: World memory heritage (1907-1922)”\]. _华人研究国际学报_, _10_(1), 131-137. https://doi.org/10.1142/S179372481800010X

Benton, G., & Liu, H. (2018). _Dear China: Emigrant letters and remittances, 1820-1980_. University of California Press.

Bhabha, H. K. (1994). _The location of culture_. Routledge.

Bitgood, S. (2013). _Attention and value: Keys to understanding museum visitors_. Left Coast Press.

Chakrabarty, D. (2000). _Provincializing Europe: Postcolonial thought and historical difference_. Princeton University Press.

Chen, J., & Cen, J. \[陈嘉添 & 岑俊伟\]. (2021). 民国时期岭南华侨的家国情怀：基于对抗战前后伍氏家族侨批的解读 \[Patriotism of overseas Chinese from Lingnan during the Republican period: Based on the interpretation of the Wu family’s overseas Chinese remittances before and after the War of Resistance\]. _暨南学报_, _1_, 91-95.

Conceicao, J. (2023). Hainanese community. Singapore Infopedia. National Library Board Singapore. https://www.nlb.gov.sg/main/article-detail?cmsuuid=b6af752e-68d5-4199-a899-2391a239a5e5

Crouch, D. (2003). Spacing, performing, and becoming: Tangles in the mundane. _Environment and Planning A_, _35_(11), 1945-1960.

Dai, Y. \[戴一峰\]. (2003). 网络化企业与嵌入性：近代侨批局的制度建构 (1850–1940) \[Networked enterprises and embeddedness: The institutional construction of modern qiao pi ju (1850-1940)\]. _中国社会经济史研究_, _1_, 70-78.

Dai, Y. \[戴一峰\]. (2004). 传统与现代:近代中国企业制度变迁的再思考——以侨批局与银行关系为中心 \[Tradition and modernity: Rethinking the transformation of modern Chinese enterprise institutions—focusing on the relationship between qiao pi ju and banks\]. _中国社会经济史研究_, _(1)_, 65-73.

Deng, D. \[邓达宏\]. (2009). 从抗战华侨捐资解读闽南华侨爱国义举 \[Interpreting the patriotic deeds of Minnan overseas Chinese from their donations during the Anti-Japanese War\]. _福建党史月刊_, _(14)_, 31-33.

Frost, M. R. (2003). Transcultural diaspora: The Straits Chinese in Singapore, 1819-1918. _Asia Research Institute Working Paper Series, No. 10_.

Gurian, E. H. (2006). _Civilizing the museum: The collected writings of Elaine Heumann Gurian_. Routledge.

Harris, L. J. (2013). Overseas Chinese remittance firms, the limits of state sovereignty, and transnational capitalism in East and Southeast Asia, 1850s-1930s. _The Journal of Asian Studies_, _74_(1), 129-151.

Hooper-Greenhill, E. (2000). _Museums and the interpretation of visual culture_. Routledge.

Hornecker, E., & Stifter, M. (2006). Learning from interactive museum installations about interaction design for public settings. In _Proceedings of the 18th Australia conference on Computer-Human Interaction_ (pp. 135-142).

Hsu, M. Y. (2000). _Dreaming of gold, dreaming of home: Transnationalism and migration between the United States and South China, 1882-1943_. Stanford University Press.

Jiao, J. \[焦建华\]. (2010). 近代跨国商业网络的构建与运作——以福建侨批网络为中心 \[Construction and operation of modern transnational commercial networks—focusing on Fujian qiao pi networks\]. _学术月刊_, _11_, 136-143.

Kirshenblatt-Gimblett, B. (1998). _Destination culture: Tourism, museums, and heritage_. University of California Press.

Kövecses, Z. (2000). _Metaphor and emotion: Language, culture, and body in human feeling_. Cambridge University Press.

Lakoff, G., & Johnson, M. (1999). _Philosophy in the flesh: The embodied mind and its challenge to Western thought_. Basic Books.

Li, H., Wang, D., Lin, K., & Bian, Y. \[李海涛, 王丹霞, 林可楠, & 边越\]. (2023). 广东省侨批档案的诚信文化传承研究 \[Research on the inheritance of trust culture in Guangdong qiao pi archives\]. _文献与数据学报_, _(3)_, 45-54.

Lin, R., & Huang, Y. \[林瑞容 & 黄雨荷\]. (2020). 以潮汕侨批文化推进本土中小学课程思政的实践探索——以《侨批文化乐园》原创课程集为例 \[Practical exploration of promoting local primary and secondary school curriculum politics with Chaoshan qiao pi culture\]. 华南师范大学.

Nacke, L. E., Kalyn, M., Lough, C., & Mandryk, R. L. (2011). Biofeedback game design: Using direct and indirect physiological control to enhance game interaction. _Proceedings of the SIGCHI Conference on Human Factors in Computing Systems_, 103-112.

Nield, R. (2015). _China’s foreign places: The foreign presence in China in the treaty port era, 1840-1943_. Hong Kong University Press.

Niu, W. \[牛文娟\]. (2013). 侨批档案中的”仁义礼智信” \[The “benevolence, righteousness, propriety, wisdom, and trustworthiness” in qiao pi archives\]. _云南档案_, _6_, 15-16.

Parry, R. (2010). _Museums in a digital age_. Routledge.

Picard, R. W. (1997). _Affective computing_. MIT Press.

Scott, J. C. (1990). _Domination and the arts of resistance: Hidden transcripts_. Yale University Press.

Serrell, B. (2015). _Exhibit labels: An interpretive approach_ (2nd ed.). Rowman & Littlefield.

Shen, H. \[沈辉\]. (2021). 华人离散群体的战争记忆——以东南亚华侨关于太平洋战争的侨批为中心 \[War memories of Chinese diaspora groups: Focusing on Southeast Asian overseas Chinese remittances about the Pacific War\]. _南洋问题研究_, _6_, 107-119.

Simon, N. (2010). _The participatory museum_. Museum 2.0.

Smith, L. (2006). Uses of heritage. Routledge.\`

Tian, L., & Xiao, W. \[田立勤 & 肖文评\]. (2015). 从侨批看民国初期梅州侨乡与印度尼西亚地区近代教育的发展 \[Observing the modern education development in Meizhou and Indonesia from Qiaopi\]. _地方文化研究_, _1_, 69-79.

Tian, M., Srinivasamurthy, A., Sandler, M., & Serra, X. (2014). A study of instrument-wise onset detection in Beijing Opera percussion ensembles. _Proceedings of the IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP 2014)_, Florence, Italy.

Tröndle, M., & Tschacher, W. (2012). The physiology of phenomenology: The effects of artworks. _Empirical Studies of the Arts_, _30_(1), 75-113.

UNESCO. (2013). _Qiaopi and Yinxin Correspondence and Remittance Documents from Overseas Chinese_. Memory of the World Register. https://www.unesco.org/en/memory-world/qiaopi-and-yinxin-correspondence-and-remittance-documents-overseas-chinese

Wang, C. \[王春泉\]. (2019). 爱国主义视域下侨批档案的高校育人价值与实现路径 \[The educational value and implementation path of qiao pi archives in universities from the perspective of patriotism\]. _档案学研究_, _3_, 76-82.

Winter, T. (2015). Heritage diplomacy. _International Journal of Heritage Studies_, _21_(10), 997-1015.

Witcomb, A. (2013). Understanding the role of affect in producing a critical pedagogy for history museums. _Museum Management and Curatorship_, _28_(3), 255-271.

Witcomb, A. (2015). Cultural pedagogies in the museum: Walking, listening and feeling. In M. Watkins, G. Noble & C. Driscoll (Eds.), _Cultural pedagogies and human conduct_ (pp. 158-170). Routledge.

Ye, Z. \[叶忠惠\]. (2022). 利用侨批开展中华传统文化教育的思考——以闽南侨批为例 \[Thoughts on using qiao pi to conduct traditional Chinese cultural education—taking Southern Fujian qiao pi as an example\]. _档案与建设_, _8_, 21-25.

Yeoh, B. S. A., & Kong, L. (2012). Singapore’s Chinatown: Nation building and heritage tourism in a multiracial city. _Localities_, _2_, 117-159.

Zheng, J. \[郑金勤\]. (2017). 侨批信笺爱国情怀 \[Patriotic sentiments in qiao pi letters\]. _东方收藏_, _(06)_, 14-15.

Zheng, M. \[郑美云\]. (2019). 侨批档案在中学历史教学中的开发和运用 \[Development and application of qiao pi archives in secondary school history teaching\] \[Master’s thesis, Fujian Normal University\]. https://link.cnki.net/doi/10.27019/d.cnki.gfjsu.2019.001647

Zhou, W. \[周文培\]. (2020). 跨国主义与新加坡琼人 \[Transnationalism and Hainanese in Singapore\] \[Doctoral thesis, Nanyang Technological University\]. https://hdl.handle.net/10356/151444

[^1]: Original text: “爱国主义视域下侨批档案的高校育人价值与实现路径”

[^2]: Original text: “华侨由对家庭、宗族、家乡的热爱，升华到对祖国的热爱”

[^3]: Original text: “本文尝试以闽南侨批为例，探讨利用侨批弘扬爱国顾家、守信重义、敬老爱幼等中华传统美德，进行爱国主义教育，推动侨乡经济、社会、文化健康和谐发展的可行路径”

[^4]: Original text: “引导学生通过潮汕侨批信件了解其有关爱党爱国、保家卫国的历史故事，了解爱国主义的多种具体表现形式，厚植爱国情怀”

[^5]: Original text: “投身于革命活动和抗日救亡运动之中”

[^6]: Original text: “仁义礼智信”

[^7]: Original text: “诚信文化”
