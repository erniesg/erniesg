...
`study-pdf-epub-heldout-v1.json` is the fixed Study acceptance corpus. Its
content hashes and expectations are immutable: a new corpus version is required
to add coverage, and a converter failure must never be hidden by removing or
rewriting a v1 case. The `heldout-v1/holdout-*` files are inert synthetic rejection inputs
covering encryption, corruption, a non-PDF container, and unsupported active
JavaScript. Run `npm run study:pdf-epub:holdout` to execute the real local
reconstructor and EPUB builders and inspect each accepted archive.
