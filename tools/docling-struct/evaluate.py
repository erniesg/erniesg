...
from pdf_links import WORD_RE, attach_words, extract_links, page_text_lines, word_boxes
...
def _running_lines(pages: list[list[str]]) -> set[str]:
    """Lines that recur near the top or bottom of at least three pages."""
...
def evaluate(pdf: Path, epub: Path, build_report: dict, struct_draft: Path | None = None) -> dict:
    ...
    for match in re.finditer(r"<figure[^>]*>(.*?)</figure>(\s*<p[^>]*class=\"caption\"[^>]*>(.*?)</p>)?", body_html, re.S):
        ...
        if "<table" in inner:
            for text in [c for c in re.findall(r"<caption>(.*?)</caption>", inner, re.S)] + ([trailing] if trailing else []):
                label = OUTPUT_CAPTION_RE.match(_strip(text))
                if label and label.group(1).lower().startswith("t"):
                    out_labels["table"].add(label.group(2))
    out_hrefs = set(re.findall(r'<a[^>]*\shref="([^"]+)"', body_html))
    ...
    completeness = {
        ...
        "resolvedSemanticTableCount": min(tables_semantic, tables_expected) if tables_expected else tables_semantic,
